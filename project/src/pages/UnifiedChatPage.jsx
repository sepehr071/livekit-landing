...
const UnifiedChatPage = () => {
  ...
  const getConnectionStatus = () => {
    if (isConnecting) return { text: "Verbindung zum KI-Assistenten...", color: "text-yellow-600" };
    if (!isConnected && !error) return { text: "Nicht verbunden", color: "text-gray-600" };
    ...
    <>
      ...
      <div className="flex justify-between items-center p-4 bg-gradient-to-br to-[#fcf4e7] from-[#dbbe9b] drop-shadow-md">
        ...
        <div className="flex gap-2">
          <span className={`mode-badge flex items-center gap-1 px-2 py-1 rounded-full text-xs transition-all duration-200 ${
            audioEnabled 
      ...
      <div className="flex-1 overflow-y-auto p-4 h-[80%] drop-shadow-md">
        ...
        <ChatAvatar
          ...
          isUserSpeaking={isUserSpeaking && !isMuted && isConnected && audioEnabled}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex justify-center">
            <button
              ...
              className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed ${
                audioEnabled 
                  ? "bg-blue-500 hover:bg-blue-600 text-white" 
                  : "bg-gray-500 hover:bg-gray-600 text-white"
              }`}
              title={audioEnabled ? "Zum Textmodus wechseln" : "Sprachmodus aktivieren"}
            >
              {audioEnabled ? <Volume2 size={20} /> : <MessageSquare size={20} />}
            </button>
          </div>
          ...
          <form onSubmit={handleSendText} className="flex items-center gap-2">
            <div className="flex-1 relative">
              <input
                ...
                placeholder={audioEnabled ? "Sprechen oder schreiben..." : "Geben Sie Ihre Nachricht ein..."}
            ...
            <button
              ...
              title={isSendingText ? "Nachricht wird gesendet..." : "Nachricht senden"}
            >
              {isSendingText ? (
